package gui.contents.games.add_game;
...
import gui.contents.games.add_game.opponent_game_result.OpponentGameResult;
...
public class AddGameController implements Initializable, FXMLController, Stageable {
    ...
    public Stage getStage() {
        ...
        stage.setTitle(dbGameItem.getIdGame().getValue() == null ? "Добавить игру" : "Изменить игру");
        try {
    ...
    private void btnSave_click(ActionEvent actionEvent) {
        ...
        for (OpponentGameResult ogr: lvOpponentGameResults.getItems()) {
            DBOpponentItem item = ogr.getController().getSelectedOpponentItem();
            if (item != null) {
                opponentNames.add(item.getName().getValue());
            }
        }
        ...
        if (lvOpponentGameResults.getItems().size() < 2) {
            errorItemIndex++;
            errorItems.append(String.format(errorItemPattern, errorItemIndex,
